...
def normalize(raw: dict):
    ...
    for i, t in enumerate(h_times):
        ...
        hp = HourlyPoint(
            ...
            feels_like_c=round(float(h_feels[i]), 1) if h_feels and h_feels[i] is not None else None,
    ...
    if current_raw:
        ...
        # current_weather time can be "2024-06-01T14:00" or "2024-06-01 14:00"
        # Normalise to "YYYY-MM-DDTHH" for matching against hourly keys
        raw_time = current_raw.get("time", "")
        normalised_time = raw_time.replace(" ", "T")  # ensure T separator
        match_key = normalised_time[:13]              # "YYYY-MM-DDTHH"

        feels_like = None
        humidity   = 0.0

        for i, t in enumerate(h_times):
            # h_times already use T separator
            if t[:13] == match_key:
                if h_feels and i < len(h_feels) and h_feels[i] is not None:
                    feels_like = round(float(h_feels[i]), 1)
                if h_humid and i < len(h_humid) and h_humid[i] is not None:
                    humidity = round(float(h_humid[i]), 1)
                break

        # Fallback: if no exact hour match, use the very first hourly entry
        if feels_like is None and h_feels:
            first_valid = next((v for v in h_feels if v is not None), None)
            if first_valid is not None:
                feels_like = round(float(first_valid), 1)
